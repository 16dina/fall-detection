...
# WORKING CODE FOR VIDEO BUT MESSES UP SAFE STATE
...
video_frames_within_duration = []
clip_duration_before = 10  # seconds before the fall
clip_duration_after = 30
video_frames_after = []
taking_video = False
clip_frames = []
...
def check_falling_time(fall_start_time, elapsed_time_states):
    ...
    if fall_start_time is not None:
        ...
        if duration_of_fall >= MIN_ELAPSED_TIME_THRESHOLD:
            ...
            elapsed_time_states.clear()
            fallen_state = False
        elif duration_of_fall < MIN_ELAPSED_TIME_THRESHOLD and fallen_state == False:
            taking_video =  False
            video_frames_after.clear()
...
#         if fallen_state:
#             clip_frames = [frame for frame in video_frames_within_duration if frame is not None]
#         else:
#             # If no fall, get frames only from the last 10 seconds
#             clip_frames = [frame for frame in video_frames_within_duration[-clip_duration_before * VIDEO_FPS:] if frame is not None]
...
def save_video_clip():
    global clip_frames
    clip_frames = video_frames_within_duration + video_frames_after
...
if results:
    ...
    for r in results:
        ...
        print("Length of BEFORE:", len(video_frames_within_duration))
        print(fallen_state)
        if taking_video:
            if len(video_frames_after) <= 300:
                ...
                send_api_call()
                taking_video = False
                video_frames_within_duration.clear()
                video_frames_after.clear()
        else:
            if len(video_frames_within_duration) <= 100:
                video_frames_within_duration.append(r.orig_img)
            else:
                video_frames_within_duration.pop(0)
        ...
        # Remove frames outside the specified duration
        # if len(video_frames_within_duration) > (clip_duration_before + clip_duration_after) * VIDEO_FPS:
        #     video_frames_within_duration.pop(0)
